...
export class DarwinRuntime extends PlatformRuntime {
  ...
  downUrl = import.meta.env.VITE_BIN_MAC_DOWNLOAD_PATH || '';
  downPythonUrl = this.downUrl + '/cpython-3.13.2+20250212-aarch64-apple-darwin-install_only.tar.gz';
  downNodeUrl = this.downUrl + '/node-v22.11.0-darwin-arm64.tar.gz';
  nodeVersion = '22.11.0';
  ...
  private getNodePath = () => {
    ...
    // TODO 临时处理一下node-v22.11.0-darwin-arm64，后面再处理
    return path.join(userDataPath, 'runtime', 'nodejs', 'node-v22.11.0-darwin-arm64','bin', 'node');
  };
