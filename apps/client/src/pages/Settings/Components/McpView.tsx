...
const McpInstaller = () => {
  const { installTool } = useMcpToolStore();
...
const McpDetail = ({ serverInfo }: { serverInfo: ServerItem | null }) => {
  ...
  // 获取服务类型的标签
  const getServiceTypeTag = (config: McpServerConfig) => {
  ...
  return (
    <div>
      <div className="flex items-center mb-2 py-4 px-6">
        {getServiceIcon(serverInfo.config)}
        <Text strong className="ml-2 text-lg break-all">{serverInfo.id}</Text>
        {getServiceTypeTag(serverInfo.config)}
      </div>
      ...
      <div className="ml-6 text-sm text-gray-500">
        {serverInfo.config && (
          <div className="mb-2">
            <Text type="secondary">
              {serverInfo.config.transport === 'sse' ? 'URL: ' : '命令: '}
            </Text>
            <Text code className="break-all">
              {serverInfo.config.transport === 'sse' 
                ? serverInfo.config.url 
                : `${serverInfo.config.command} ${serverInfo.config.args?.join(' ')}`}
            </Text>
          </div>
        )}
        
        <Collapse ghost className="mt-2" defaultActiveKey={['1']} items={[
          {
            key: '1',
            label: `可用工具 (${serverInfo.tools?.length})`,
            children: (
...
export default function McpView() {
  const { selectedServer, servers } = useMcpToolStore();
  const [ serverList, setServerList ] = useState<Array<ServerItem>>([]);
  const [ selectedServerInfo, setSelectedServerInfo ] = useState<ServerItem | null>(null);
  ...
  useEffect(() => {
    if (selectedServer) {
      const serverInfo = serverList.find(item => item.id === selectedServer);
      if (serverInfo) {
  ...
  return (
    <div className="h-full">
      {selectedServer ? <McpDetail serverInfo={selectedServerInfo} /> : <McpInstaller />}
    </div>
  );
}
